...
//      test_meta<T> specialisations and confirm their reported size/align/
//      qualified-name agree with what the C++ language model says about
//      the original types.
//
// If the generator stops emitting a test_meta<TestStruct> specialisation,
...
int main()
{
    ...
    static_assert(std::is_same_v<TS::type, TestStruct>,
                  "test_meta::type must be TestStruct");
    ...
    check(TS::name == std::string_view{"TestStruct"},
          "test_meta<TestStruct>::name");
    check(TS::full_qualified_name == std::string_view{"TestStruct"},
          "test_meta<TestStruct>::full_qualified_name");

    // ---------- test_meta<TestClass> (abstract) ----------
    using TC = test_meta<TestClass>;
    ...
    // ---------- test_meta<test_ns::TestClass2> ----------
    using TC2 = test_meta<test_ns::TestClass2>;
    static_assert(TC2::size  == sizeof(test_ns::TestClass2),
                  "TestClass2 size");
    check(TC2::full_qualified_name == std::string_view{"test_ns::TestClass2"},
          "test_meta<TestClass2>::full_qualified_name preserves namespace");

    // ---------- test_meta<test_ns::test_ns_2::TestClass3> ----------
    using TC3 = test_meta<test_ns::test_ns_2::TestClass3>;
    check(TC3::full_qualified_name == std::string_view{"test_ns::test_ns_2::TestClass3"},
          "test_meta<TestClass3>::full_qualified_name preserves nested namespace");
