...
The repository includes a reference template at `templates/test_meta.template.inja`:
...
This generates a compile-time `test_meta<T>` specialization for each reflected struct/class, exposing name, size, alignment, and field types as a `std::tuple`.
