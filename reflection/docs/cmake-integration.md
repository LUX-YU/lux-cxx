...
The `reflection` module provides CMake functions that automate the parse → generate → compile pipeline. This document covers the `add_meta()`, `meta_add_files()`, and `target_add_meta()` functions defined in `cmake/meta.cmake`.

## Quick Start

```cmake
include(${CMAKE_SOURCE_DIR}/reflection/cmake/meta.cmake)
