...
A C++ reflection system based on **libclang**, designed for game engines and other applications that need runtime type introspection, editor tooling, and code generation.
...
| `Result<T>` / `ReflectionError` | ⚠️ | Only used by `GeneratorHelper::fetchIncludePaths`; the rest of the API still uses bool/exceptions |
