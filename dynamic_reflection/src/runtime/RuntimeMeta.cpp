...
namespace lux::cxx::dref::runtime
{
    ...
    {
        static BuiltinTypeRuntimeMeta s_builtin_types[] = {
            ...
            { "float16",          lux::cxx::algorithm::fnv1a("float16"),          0,                        BuiltinType::EBuiltinKind::FLOAT16 },
        ...
        // 计算 hash 并注册
        for (auto& bt : s_builtin_types) {
            // 使用 fnv1a 或任何你喜欢的哈希函数
            bt.hash = lux::cxx::algorithm::fnv1a(bt.name);
            registerMeta(&bt);
        }
    }
    ...
    void RuntimeRegistry::registerMeta(BuiltinTypeRuntimeMeta* meta)
    {
        builtin_meta_list_.push_back(meta);
        size_t index = builtin_meta_list_.size() - 1;
        MetaInfo info{
            ERuntimeMetaType::BUILTIN,
			index,
			static_cast<void*>(meta)
		};
		meta_map_[meta->hash] = info;
