...
namespace lux::cxx::dref::runtime
{
	...
	public:
		...
		{
			return TFnv1a(nameof::nameof_full_type<T>()) == hash;
		}
	...
	private:
		...
		inline bool _parameter_check(const std::vector<TypeMeta*>& parameters_list)
		{
			constexpr size_t _type_id = TFnv1a(nameof::nameof_full_type<T>());
			return  TTypeCheck<T>(parameters_list[I]->type_id) &&
		...
		inline bool _args_check(const std::vector<TypeMeta*>& parameters_list, T&& arg, Args&&... args)
		{
			...
			return arg.type_id == parameters_list[I]->type_id &&
				_args_check<I + 1>(std::forward<Args>(args)...);
		}
...
#define LUX_ARG(type_name, value)\
	::lux::cxx::dref::runtime::FunctionCallArgument{::lux::cxx::dref::runtime::TFnv1a(#type_name), std::addressof(value)}
