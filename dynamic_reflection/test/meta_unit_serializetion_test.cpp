...
int main(int argc, char* argv[])
{
	...
	auto binaray_data = lux::cxx::dref::MetaUnit::serializeBinary(data);
	auto new_unit     = lux::cxx::dref::MetaUnit::deserializeBinary(binaray_data.data(), binaray_data.size());

    using namespace lux::cxx::lan_model;

    const auto& type_meta_list = new_unit.typeMetaList();
    ...
    for (const auto& decl : new_unit.markedClassDeclarationList())
    {
        show_declaration_info(&decl);
        std::cout << std::endl;
    }

    for (const auto& decl : new_unit.markedEnumerationDeclarationList())
    {
        show_declaration_info(&decl);
        std::cout << std::endl;
    }

    for (const auto& decl : new_unit.markedFunctionDeclarationList())
    {
        show_declaration_info(&decl);
        std::cout << std::endl;
    }
    ...
    for (const auto& decl : new_unit.unmarkedClassDeclarationList())
    {
        show_declaration_info(&decl);
        std::cout << std::endl;
    }

    for (const auto& decl : new_unit.unmarkedEnumerationDeclarationList())
    {
        show_declaration_info(&decl);
        std::cout << std::endl;
    }

    for (const auto& decl : new_unit.unmarkedFunctionDeclarationList())
    {
        show_declaration_info(&decl);
        std::cout << std::endl;
    }
